...
public class ArtistsListPanel extends JPanel{
		
		private static DefaultTableModel model = new DefaultTableModel();
		private static JTable table;
		private JScrollPane sp;
		
		public ArtistsListPanel(MainFrame frame) {
			super();
		...
		public static void addRow(String artist) {
			if(model.getRowCount() == 0) {
				...
				for(int i = 0; i < table.getRowCount(); i++) {
					if(table.getModel().getValueAt(i, 0).equals(artist)) {
						duplicateRow = i;
						break;
					}
					if (duplicateRow == -1)
